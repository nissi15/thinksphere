    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl glass-light-card flex items-center justify-center">
            <User className="w-7 h-7 text-slate-400" />
          </div>
          <div>
            <p className="font-gotham text-sm text-slate-400">Welcome back,</p>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl glass-light-card">
          ...
          <span className="text-xs font-gotham font-semibold text-slate-600">
            Active Participant
          </span>
        </div>
      </div>
      ...
      <motion.div
        ...
        className="glass-light-card rounded-2xl p-5 mb-6 flex items-center gap-4"
      >
        <div className="w-12 h-12 rounded-xl bg-[#0891b2]/10 flex items-center justify-center shrink-0">
          <Clock className="w-6 h-6 text-[#0891b2]/70" />
        </div>
        <div className="flex-1">
          <p className="font-gotham text-base font-bold text-slate-700">
        ...
        <div className="text-right shrink-0 hidden sm:block">
          <p className="text-xs font-gotham text-slate-400">Next session</p>
          <p className="text-sm font-gotham font-semibold text-slate-600">March 22, 2026</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {skills.map((s) => (
              <div key={s.name} className="glass-light-card rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-gotham text-sm font-bold text-slate-700">
                    {s.name}
                  </span>
                  {s.earned ? (
                    <CheckCircle2 className="w-4 h-4 text-[#0891b2]/70" />
                  ) : (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="stat-font text-xs text-slate-400">{s.level}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div className="space-y-2">
            {sessions.map((s, i) => (
              <div
                key={i}
                className={`glass-light-card rounded-xl p-4 flex items-center gap-4 ${
              ...
              >
                ...
                >
                  {s.status === "completed" ? (
                    ...
                    <Calendar className="w-5 h-5 text-[#0891b2]/60" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-gotham text-sm font-semibold text-slate-600 truncate">
                    {s.title}
                  </p>
      ...
      >
        <Link
          ...
          className="inline-flex items-center gap-2 font-gotham text-sm font-semibold px-6 py-3 rounded-xl bg-[#0891b2] text-white hover:bg-[#0891b2]/90 transition-colors"
        >
